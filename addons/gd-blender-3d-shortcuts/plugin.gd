@tool
extends EditorPlugin # Node

enum SESSION {TRANSLATE, ROTATE, SCALE, NONE}

const Utils = preload("Utils.gd")
const PieMenu = preload("scenes/pie_menu/PieMenu.gd")
const PieMenuScene = preload("scenes/pie_menu/PieMenu.tscn")
const PieMenuGroup = preload("scenes/pie_menu/PieMenuGroup.gd")
const PieMenuGroupScene = preload("scenes/pie_menu/PieMenuGroup.tscn")

const DEFAULT_LINE_COLOR = Color.WHITE
# [name, value]
const DEBUG_DRAW_OPTIONS: Array[Variant] = [
	["Normal", 0],
	["Unshaded", 1],
	["Lighting", 2],
	["Overdraw", 3],
		...
		[
			[
				...
				[
					...
					["Directional Shadow Splits", 14]
				]
			],
			["Lights", [["Omni Lights Cluster", 20], ["Spot Lights Cluster", 21]]],
			["VoxelGI", [["VoxelGI Albedo", 6], ["VoxelGI Lighting", 7], ["VoxelGI Emission", 8]]],
			["SDFGI", [["SDFGI", 16], ["SDFGI Probes", 17], ["GI Buffer", 18]]],
			["Environment", [["SSAO", 12], ["SSIL", 13]]],
			["Decals", [["Decal Atlas", 15], ["Decal Cluster", 22]]],
			[
				"Others",
				[
					["Normal Buffer", 5],
...
var translate_snap_line_edit: EditorSpinSlider
var rotate_snap_line_edit: EditorSpinSlider
var scale_snap_line_edit: EditorSpinSlider
var local_space_button: Button
var snap_button: Button
var overlay_control: Control
# Array[Node3DEditorViewport]
...
var _cumulative_center_offset := 0.0
var _max_x := 0.0
var _min_x := 0.0
...
func _init() -> void:
	axis_im_material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
...
func _ready() -> void:
	# Node3DEditor
	var spatial_editor := Utils.get_3d_editor(EditorInterface.get_base_control())
	...
	if snap_dialog_line_edits.size() >= 3:
		...
		_connect_snap_control(translate_snap_line_edit, SESSION.TRANSLATE)
		_connect_snap_control(rotate_snap_line_edit, SESSION.ROTATE)
		_connect_snap_control(scale_snap_line_edit, SESSION.SCALE)
	else:
		push_warning("[Blender 3D Shortcuts] Could not find snap dialog line edits.")

	local_space_button = Utils.get_spatial_editor_local_space_button(spatial_editor)
	assert(not local_space_button.toggled.is_connected(_on_local_space_button_toggled))
	if local_space_button.toggled.connect(_on_local_space_button_toggled) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect to local_space_button.toggled")

	snap_button = Utils.get_spatial_editor_snap_button(spatial_editor)
	assert(not snap_button.toggled.is_connected(_on_snap_button_toggled))
	if snap_button.toggled.connect(_on_snap_button_toggled) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect to snap_button.toggled")

	debug_draw_pie_menu = PieMenuGroupScene.instantiate() as PieMenuGroup
	assert(is_instance_valid(debug_draw_pie_menu))
	debug_draw_pie_menu.populate_menu(DEBUG_DRAW_OPTIONS, PieMenuScene.instantiate() as PieMenu)
	debug_draw_pie_menu.theme_source_node = spatial_editor
	assert(not debug_draw_pie_menu.item_focused.is_connected(_on_PieMenu_item_focused))
	if debug_draw_pie_menu.item_focused.connect(_on_PieMenu_item_focused) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect to debug_draw_pie_menu.item_focused")

	assert(not debug_draw_pie_menu.item_selected.is_connected(_on_PieMenu_item_selected))
	if debug_draw_pie_menu.item_selected.connect(_on_PieMenu_item_selected) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect to debug_draw_pie_menu.item_focused")

	# Node3DEditorViewportContainer
	var spatial_editor_viewport_container := Utils.get_spatial_editor_viewport_container(
		spatial_editor
	)
	if is_instance_valid(spatial_editor_viewport_container):
		# Node3DEditorViewport
		spatial_editor_viewports = Utils.get_spatial_editor_viewports(
			spatial_editor_viewport_container
		)
...
func _input(event: InputEvent) -> void:
			...
			match input_event_key.keycode:
				KEY_Z:
					...
					if is_instance_valid(focus):
						var focus_parent := focus.get_parent_control()
						if is_instance_valid(focus_parent):
							# This is fragile to name changes or the control being placed another level deeper internally
							if "Node3DEditorViewport" in focus_parent.name:
	...
	if input_event_mouse_motion != null:
		if current_session != SESSION.NONE and is_instance_valid(overlay_control):
			...
			if not rect.has_point(local_mouse_pos):
				...
				var warp_pos: Variant = Utils.infinite_rect(
					rect, local_mouse_pos, -input_event_mouse_motion.velocity.normalized() * rect.size.length()
				)
				if typeof(warp_pos) == TYPE_VECTOR2:
					var warp_pos_vector: Vector2 = warp_pos
					Input.warp_mouse(overlay_control.global_position + warp_pos_vector)
					_is_warping_mouse = true


func _connect_snap_control(control: EditorSpinSlider, session: SESSION) -> void:
	if not control.value_changed.is_connected(_update_snap_value):
		if control.value_changed.connect(_update_snap_value.bind(session)) != OK:
			push_error("[Blender 3D Shortcuts] Failed to connect to value_changed")


func _update_snap_value(value: float, session: SESSION) -> void:
	match session:
		SESSION.TRANSLATE:
...
func show_debug_draw_pie_menu() -> void:
	var spatial_editor_viewport := Utils.get_focused_spatial_editor_viewport(
		spatial_editor_viewports
	)
	overlay_control = (
...
func _get_filtered_selected_nodes() -> Array[Node3D]:
...
func _handles(object: Object) -> bool:
	var nodes := _get_filtered_selected_nodes()
	if object is Node3D:
		_is_editing = nodes.size() > 0
		return _is_editing

	# Explicitly handle MultiNodeEdit, otherwise, it will activate when selected Resource
...
func _edit(_object: Object) -> void:
	...
	if is_instance_valid(scene_root):
		...
		if not is_instance_valid(axis_mesh_inst) or axis_mesh_inst.is_queued_for_deletion():
			axis_mesh_inst = MeshInstance3D.new()
			axis_mesh_inst.mesh = axis_im
			axis_mesh_inst.material_override = axis_im_material

		var current_parent: Node = axis_mesh_inst.get_parent()
		var desired_parent := scene_root.get_parent()
		if is_instance_valid(desired_parent):
			if not is_instance_valid(current_parent):
				desired_parent.add_child(axis_mesh_inst)
			else:
				if current_parent != desired_parent:
					current_parent.remove_child(axis_mesh_inst)
					desired_parent.add_child.call_deferred(axis_mesh_inst)
...
func _forward_3d_draw_over_viewport(overlay: Control) -> void:
	if current_session == SESSION.NONE:
		var overlay_label_parent := overlay_label.get_parent()
		if is_instance_valid(overlay_label_parent):
			overlay_label_parent.remove_child(overlay_label)
	...
	var snapped_text := "snapped_text" if is_snapping else ""
	var global_or_local_text := "global" if is_global else "local"
	var along_axis_text := ""
	if not constraint_axis.is_equal_approx(Vector3.ONE):
		if constraint_axis.x > 0:
			along_axis_text = "X"
		if constraint_axis.y > 0:
			along_axis_text += ", Y" if along_axis_text.length() else "Y"
		if constraint_axis.z > 0:
			along_axis_text += ", Z" if along_axis_text.length() else "Z"

	if along_axis_text.length():
		along_axis_text = "along " + along_axis_text

	if not is_instance_valid(overlay_label.get_parent()):
	...
	match current_session:
		...
		SESSION.SCALE:
			...
			overlay_label.text = (
				...
				% [scale.x, scale.y, scale.z, global_or_local_text, along_axis_text, snapped_text]
			)
	...
	var screen_origin := (
		...
		else _camera.unproject_position(pivot_point)
	)
	Utils.draw_dashed_line(
		overlay, screen_origin, overlay.get_local_mouse_position(), line_color, 1, 5, true, true
	)
...
func text_transform(text: String) -> void:
	...
	match current_session:
		...
		SESSION.ROTATE:
			_applying_transform.basis = Basis().rotated(
	...
	var nodes := _get_filtered_selected_nodes()
	var transform := _applying_transform
	if (
		is_global
		or (constraint_axis.is_equal_approx(Vector3.ONE) and current_session == SESSION.TRANSLATE)
	):
		transform.origin += pivot_point
		Utils.apply_global_transform(nodes, transform, _cache_transforms)
	else:
		Utils.apply_transform(nodes, transform, _cache_global_transforms)
...
func mouse_transform(event: InputEventMouseMotion) -> void:
	...
	if axis_count == 2:
		...
		if is_single_node and not is_global:
			normal = first_node.global_transform.basis * normal
	...
	if not is_global and is_single_node and axis_count < 3:
		var normalized_first_node_basis := first_node.global_transform.basis.scaled(
			Vector3.ONE / first_node.global_transform.basis.get_scale()
		)
		world_pos = world_pos * normalized_first_node_basis
	...
	if _max_x == 0:
		...
		_min_x = _max_x - (_max_x - screen_origin.x) * 2.0

	var center_value := 2.0 * ((event.position.x - _min_x) / (_max_x - _min_x)) - 1.0
	if _last_center_offset == 0:
		_last_center_offset = center_value

	var center_offset := center_value - _last_center_offset
	center_offset = snappedf(center_offset, 0.001)
	if _is_warping_mouse:
		center_offset = 0

	_cumulative_center_offset += center_offset
		...
		match current_session:
				...
				if is_snapping:
					var snap := (
						...
						* (
							...
							else translate_snap * precision_factor
						)
					)
			...
			SESSION.ROTATE:
				...
				if not rotation_axis.is_equal_approx(Vector3.ZERO):
					_editing_transform.basis = _editing_transform.basis.rotated(
						rotation_axis, angle_offset
					)
					...
					if is_snapping:
						var snap := (
							...
							* (
								rotate_snap
								if not precision_mode
								else rotate_snap * precision_factor
							)
						)
			...
			SESSION.SCALE:
				if constraint_axis.x:
					_editing_transform.basis.x = Vector3.RIGHT * (1 + _cumulative_center_offset)

				if constraint_axis.y:
					_editing_transform.basis.y = Vector3.UP * (1 + _cumulative_center_offset)

				if constraint_axis.z:
					_editing_transform.basis.z = Vector3.BACK * (1 + _cumulative_center_offset)

				_applying_transform.basis = _editing_transform.basis
				if is_snapping:
					var snap := (
						Vector3.ONE
						* (scale_snap if not precision_mode else scale_snap * precision_factor)
					)
...
func cache_selected_nodes_transforms() -> void:
	var nodes := _get_filtered_selected_nodes()
	var inverse_pivot_transform := Transform3D.IDENTITY.translated(pivot_point).affine_inverse()
	for index in nodes.size():
		var node := nodes[index]
		_cache_global_transforms.append(node.global_transform)
		_cache_transforms.append(inverse_pivot_transform * node.global_transform)
...
func start_session(session: SESSION, camera: Camera3D, event: InputEventKey) -> void:
	var nodes := _get_filtered_selected_nodes()
	if nodes.size() == 0:
		return
...
func commit_session() -> void:
	...
	undo_redo.create_action(str(SESSION.keys()[current_session]).to_lower().capitalize())
	var transform := _applying_transform
	if (
		is_global
		or (constraint_axis.is_equal_approx(Vector3.ONE) and current_session == SESSION.TRANSLATE)
		...
		transform.origin += pivot_point
		undo_redo.add_do_method(Utils, &"apply_global_transform", nodes, transform, _cache_transforms)
	else:
		undo_redo.add_do_method(Utils, &"apply_transform", nodes, transform, _cache_global_transforms)
...
func confirm_delete_selected_nodes() -> void:
	...
	# Setting dialog text dynamically depending on the selection to mimic Godot's normal behavior.
	...
	assert(not popup.canceled.is_connected(popup.queue_free))
	if popup.canceled.connect(popup.queue_free) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect popup.canceled")

	assert(not popup.confirmed.is_connected(delete_selected_nodes))
	if popup.confirmed.connect(delete_selected_nodes) != OK:
		push_error("[Blender 3D Shortcuts] Failed to connect popup.confirmed")

	assert(not popup.confirmed.is_connected(popup.queue_free))
...
func sync_settings() -> void:
	if translate_snap_line_edit:
		translate_snap = translate_snap_line_edit.value

	if rotate_snap_line_edit:
		rotate_snap = deg_to_rad(rotate_snap_line_edit.value)

	if scale_snap_line_edit:
		scale_snap = scale_snap_line_edit.value / 100.0
...
func switch_display_mode(debug_draw: int) -> void:
	var spatial_editor_viewport := Utils.get_focused_spatial_editor_viewport(
		spatial_editor_viewports
	)
	if spatial_editor_viewport:
		var viewport := Utils.get_spatial_editor_viewport_viewport(spatial_editor_viewport)
		viewport.set(&"debug_draw", debug_draw)


# Repeatedly applying an axis will toggle is_global, like pressing xx, yy or zz in blender
...
func toggle_input_string_sign() -> void:
...
func trim_input_string() -> void:
	_input_string = _input_string.substr(0, _input_string.length() - 1)
	input_string_changed()


func append_input_string(text: String) -> bool:
...
func input_string_changed() -> void:
	if not _input_string.is_empty():
...
func get_constraint_axis_count() -> int:
	var axis_count := 3
	if constraint_axis.x == 0:
		axis_count -= 1
...
func set_constraint_axis(vector: Vector3) -> void:
	revert()
	if constraint_axis != vector:
		constraint_axis = vector
		draw_axises()
	else:
		constraint_axis = Vector3.ONE
...
func set_is_global(new_value: bool) -> void:
	if is_global != new_value:
		if is_instance_valid(local_space_button):
			local_space_button.button_pressed = !new_value
			...
			if is_global:
				...
				for node in nodes:
					Utils.draw_axis(
						...
						node.global_transform.basis * axis,
						axis_length,
						color
					)
